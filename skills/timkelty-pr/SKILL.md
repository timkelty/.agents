...
3. If the `copilot-code-review-settings` skill is available, invoke it for the chosen base branch before changing the pull request's draft state. If automatic reviews do not cover draft pull requests and new pushes, recommend enabling that coverage, then continue; configuration is not itself a gate. If the skill or settings are unavailable, continue and report that the check was skipped or inconclusive.
...
8. Repeat this review loop against one recorded head commit until every required lane is clean on the same current head:
   ...
   - Rely only on repository-configured automatic Copilot Code Review; never request it manually. When the reported settings say an automatic review should cover the pull request's current state and head, treat that review as a required lane: use the host's bounded wait or resume mechanism, address its actionable comments, and do not finish while it is pending. If no review arrives during the available wait, report it as the exact remaining gate instead of waiting indefinitely. If the settings are unavailable, not configured, or do not cover the current head, report that and continue without treating the missing Copilot review as a failed gate.
...
10. Immediately before readiness, fetch the remote base again. If its tip advanced, rebase, push, and restart the review and check loops. Refresh the pull request title and description to match the final diff and validation. Mark the pull request ready for human review only when it is rebased, mergeable, scoped correctly, validated, every check and review lane available while draft is clean on the same current head, and no other gate remains. If a required check or automatic Copilot review is configured to trigger only when a draft becomes ready, mark it ready after the pre-ready gates pass, then wait for those results and address failures or actionable feedback before finishing. If a fix changes the head and this workflow marked the pull request ready solely to trigger those gates, return it to draft and repeat the ready transition; otherwise ask before converting an existing ready pull request to draft. Use the bounded wait behavior from step 8 and report any result that does not arrive as the exact remaining gate.
