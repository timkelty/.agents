...
10. Immediately before readiness, fetch the remote base again. If its tip advanced, rebase, push, and restart the review and check loops. Refresh the pull request title and description to match the final diff and validation. Mark the pull request ready for human review only when it is rebased, mergeable, scoped correctly, validated, all required checks pass, and every required pre-ready review lane is clean on the same current head. If repository settings trigger Copilot only when a draft becomes ready, mark it ready after those gates pass, then use the bounded wait behavior from step 8 and address any resulting review before finishing. Otherwise leave it draft and state the exact remaining gate.
